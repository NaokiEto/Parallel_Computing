...
ifeq ($(OS),$(filter $(OS), lh xp))
ifdef BOLT
CFLAGS := $(subst /MT,/MD,$(CFLAGS)) /D "_UNICODE" /D "UNICODE"
endif
endif
...
# OPENCV library  files  required to build any target for OpenCV 
ifneq ($(OS), lnx)
ifdef OPENCV
ifeq ($(BITS), 32)
ifeq  ($(CL_LIB_VER),vc100)
LIBDIRS  += "$(OPENCV_DIR)/x86/vc10/lib"  
else
LIBDIRS  += "$(OPENCV_DIR/x86/vc11/lib"  
endif
else
ifeq  ($(CL_LIB_VER),vc100)
LIBDIRS  += "$(OPENCV_DIR)/x64/vc10/lib"
else
LIBDIRS  += "$(OPENCV_DIR)/x64/vc11/lib"
endif
endif
endif
endif

# BoltUtil static library is required to build any target for Bolt
ifdef BOLT
ifndef INDEPENDENT_SAMPLE
ifdef release
SDK_UTIL_LIBS := $(DEPTH)/samples/bolt/BoltUtil/$(OUTPUTDIR)/$(LIBPREFIX)BoltUtil$(STATIC_LIBSUFFIX)
else
SDK_UTIL_LIBS := $(DEPTH)/samples/bolt/BoltUtil/$(OUTPUTDIR)/$(LIBPREFIX)BoltUtil$(STATIC_LIBSUFFIX)
endif
endif
...
# TBB library and include files  required to build any target for Bolt TBB config
ifdef BOLT
ifdef tbb
ifeq ($(BITS), 32)
ifeq  ($(CL_LIB_VER),vc100)
LIBDIRS  += "$(TBB_ROOT)/lib/ia32/vc10"  
else
LIBDIRS  += "$(TBB_ROOT)/lib/ia32/vc11"  
endif
else
ifeq  ($(CL_LIB_VER),vc100)
LIBDIRS  += "$(TBB_ROOT)/lib/intel64/vc10"
else
LIBDIRS  += "$(TBB_ROOT)/lib/intel64/vc11"
endif
endif
INCLUDEDIRS += "$(TBB_ROOT)/include"  
CFLAGS += /D "ENABLE_TBB"
endif
endif

# AmpUtil static library is required to build any target for C++Amp
ifdef CPPAMP
ifndef INDEPENDENT_SAMPLE
ifdef release
SDK_UTIL_LIBS := $(DEPTH)/samples/C++Amp/AmpUtil/$(OUTPUTDIR)/$(LIBPREFIX)AmpUtil$(STATIC_LIBSUFFIX)
else
SDK_UTIL_LIBS := $(DEPTH)/samples/C++Amp/AmpUtil/$(OUTPUTDIR)/$(LIBPREFIX)AmpUtil$(STATIC_LIBSUFFIX)
endif
endif
...
ifdef OPENCV
	INSTALL_BIN_DIR = $(DEPTH)/samples/opencv/bin
else
ifdef BOLT
	INSTALL_BIN_DIR = $(DEPTH)/samples/bolt/bin
else
ifdef CAL
	INSTALL_BIN_DIR = $(DEPTH)/samples/cal/bin
endif
ifdef CPPAMP
	INSTALL_BIN_DIR = $(DEPTH)/samples/C++Amp/bin
endif
endif
endif
endif
...
ifdef OPENCV
	ADDSDKLIBS += $(foreach f, $(OPENCV_LLIBS), $(LD_LIBLINK_PREFIX)$(f)$(LD_LIBLINK_SUFFIX))
    ifeq ($(OS), lnx)
	ADDSDKLIBDIRS += $(foreach f, $(OPENCV_LIBDIRS), $(LD_LIBDIR_FLAG)$(f)) $(LD_LIBDIR_FLAG)"$(OPENCV_DIR)/lib"
	else
	ADDSDKLIBDIRS += $(LD_LIBDIR_FLAG)$(OPENCV_LIBDIRS)
	endif
	ADDSDKINCDIRS += $(C_INCLUDE_FLAG)$(OPENCV_INCLUDEDIRS) $(C_INCLUDE_FLAG)"$(OPENCV_DIR)/include"
else
ifdef BOLT
	ADDSDKLIBS += $(foreach f, $(BOLT_LLIBS), $(LD_LIBLINK_PREFIX)$(f)$(LD_LIBLINK_SUFFIX))
	ADDSDKLIBDIRS += $(LD_LIBDIR_FLAG)$(BOLT_LIBDIRS) $(LD_LIBDIR_FLAG)"$(BOLTLIB_DIR)/$(BOLT_LIBDIR_NAME)"
	ADDSDKINCDIRS += $(C_INCLUDE_FLAG)$(BOLT_INCLUDEDIRS) $(C_INCLUDE_FLAG)"$(BOLTLIB_DIR)/include" $(C_INCLUDE_FLAG)$(DEPTH)/include
else
ifdef CAL
...
ifeq ($(OS), lnx)
ifdef release
	export LD_LIBRARY_PATH:=$(AMDAPPSDKROOT)/lib/$(CPU_ARCH):$(AMDAPPSDKSAMPLESROOT)/lib/$(CPU_ARCH):$(OPENCV_DIR)/lib:$(LD_LIBRARY_PATH)
else
	export LD_LIBRARY_PATH:=$(AMDAPPSDKROOT)/lib/$(CPU_ARCH):$(AMDAPPSDKSAMPLESROOT)/lib/$(CPU_ARCH):$(OPENCV_DIR)/lib:$(LD_LIBRARY_PATH)
endif
endif
...
ifdef SAMPLE_EXE
	...
	rm -rf $(DEPDIR)
	rm -f $(INSTALL_BIN_DIR)/$(CPU_ARCH)/$(EXE_OUTPUT)

ifdef CLFILES
	for f in $(CLFILES); do \
		rm -f $(INSTALL_BIN_DIR)/$(CPU_ARCH)/$$f; \
	done
endif
...
ifeq ($(LIB_TARGET), BoltUtil)
SDK_UTIL_LIBS := 
endif

ifeq ($(LIB_TARGET), AmpUtil)
SDK_UTIL_LIBS := 
endif
...
$(info Error: Environment variable LIBAPARAPI is not defined)
endif
endif

# Remove all the *.obj files
clean_objs:
ifdef SUBDIRS
	for i in $(SUBDIRS); do make -C $$i clean_objs; done
endif
ifdef SAMPLE_EXE
	rm -f -r $(BUILDDIR)
	rm -rf $(DEPDIR)
