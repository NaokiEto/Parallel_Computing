...
ifdef INTEL
  ...
  CC                := cl
  CLVER             := $(shell cl 2>&1 1>/dev/null)
  ifeq ($(findstring 16., $(CLVER)), 16.)
    CL_LIB_VER      := vc100
  else
  ifeq ($(findstring 17., $(CLVER)), 17.)
    CL_LIB_VER      := vc110
  else
    CL_LIB_VER      :=
  endif
...
ifdef INTEL
  ...
  ifdef release
  OPENCV_LLIBS      := opencv_core$(OCVCL_VER) opencv_imgproc$(OCVCL_VER) opencv_calib3d$(OCVCL_VER) opencv_video$(OCVCL_VER) opencv_features2d$(OCVCL_VER) opencv_ml$(OCVCL_VER) opencv_highgui$(OCVCL_VER) opencv_objdetect$(OCVCL_VER) opencv_contrib$(OCVCL_VER) opencv_legacy$(OCVCL_VER) opencv_ocl$(OCVCL_VER) OpenCL
  else
  OPENCV_LLIBS      := opencv_core$(OCVCL_VER)d opencv_imgproc$(OCVCL_VER)d opencv_calib3d$(OCVCL_VER)d opencv_video$(OCVCL_VER)d opencv_features2d$(OCVCL_VER)d opencv_ml$(OCVCL_VER)d opencv_highgui$(OCVCL_VER)d opencv_objdetect$(OCVCL_VER)d opencv_contrib$(OCVCL_VER)d opencv_legacy$(OCVCL_VER)d opencv_ocl$(OCVCL_VER)d OpenCL
  endif
  ifeq ($(BITS), 64)
  ...
  OPENCL_LLIBS := OpenCL
  ifdef release
	BOLT_LLIBS   := OpenCL clBolt.runtime.$(CL_LIB_VER)
  else
	BOLT_LLIBS   := OpenCL clBolt.runtime.$(CL_LIB_VER).debug
  endif
  CPPAMP_LLIBS   := 
else
...
ifdef MINGW
  ...
  MANIFEST_SUFFIX  := 
  OPENCL_LLIBS := OpenCL
  OPENCV_LLIBS      := opencv_core opencv_imgproc opencv_calib3d opencv_video opencv_features2d opencv_ml opencv_highgui opencv_objdetect opencv_contrib opencv_legacy opencv_ocl OpenCL
  BOLT_LLIBS   := 
  CPPAMP_LLIBS := 
ifdef MINGW
  ifeq ($(BITS), 64)
...
INCLUDEDIRS = $(DEPTH)/samples/opencl/SDKUtil/include $(DEPTH)/samples/bolt/BoltUtil/include $(DEPTH)/samples/C++Amp/AmpUtil/include $(DEPTH)/samples/opencv/OpenCVUtils
OPENCL_INCLUDEDIRS = "$(AMDAPPSDKROOT)/include"
OPENCV_INCLUDEDIRS = "$(AMDAPPSDKROOT)/include"
BOLT_INCLUDEDIRS = "$(AMDAPPSDKROOT)/include"
CAL_INCLUDEDIRS = "$(AMDAPPSDKROOT)/include/CAL"
CPPAMP_INCLUDEDIRS = 
OPENCL_LIBDIRS = "$(AMDAPPSDKROOT)/lib/$(CPU_ARCH)"
OPENCV_LIBDIRS = "$(AMDAPPSDKROOT)/lib/$(CPU_ARCH)"
BOLT_LIBDIRS = "$(AMDAPPSDKROOT)/lib/$(CPU_ARCH)"
...
ifeq ($(BITS), 64)
BOLT_LIBDIR_NAME = lib64
else
BOLT_LIBDIR_NAME = lib
endif
