...
void* thread_function(void* ptr)
{
    ...
    bounds[0] = -10.0 + 20.0 * double(FuncThreadId * (FuncMpiRank - 1.0)) / double(FuncNumThreads * FuncMpiSize);
    bounds[1] = -10.0 + 20.0 * double(FuncThreadId * (FuncMpiRank - 1.0) + 1.0) / double(FuncNumThreads * FuncMpiSize);
    bounds[2] = -10;
    bounds[3] = 10;
    bounds[4] = -10;
    bounds[5] = 10;
...
int main(int argc, char *argv[])
{
    ...
    if (MPI_rank >= 1)
    {
        ...
        thread_data_array[i].mpiSize = MPI_size;
