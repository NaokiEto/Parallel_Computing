...
*        pthreads (or POSIX Threading). It converts each piece to metaimage 
*        data so that vtkMarchingCubes class can be applied, applies marching 
*        cubes to each thread, outputs the resulting vtk data, and then 
*        conglomerates the data into 1 vtk file.
 ...
 * that was found, the number of total threads, and vtk data that has been 
 * outputted by marching cubes. This will be useful for determining the bounds 
 * in the vtk data for each particular thread, as well as in the 
 * conglomeration of all the pieces of vtk data later on.
*/
 ...
 * well as pthreads. The resultant image is saved as metaimage data. 
 * vtkMarchingCubes is applied to these pieces of data, the resulting 
 * vtk data is saved as a member of the struct.
*/
void* thread_function(void* ptr)
{
    /* This is a struct variable that will be useful later on for determining
       the bounds in the vtkpolydata for each thread, as well as in the 
