...
void* thread_function(void* ptr)
{
    ...
    if(NewPtr->threadId > 0)
        NumOfCharPD = 18 + (int) log10(NewPtr->threadId);
...
int main(int argc, char *argv[])
{
	...
	for (int f = 0; f < size; f++) {      
        ...
        extension2 = (char) f + ".vtk";

        VT_ON();
        VT_USER_START("Region 1");

	    // setup search parameters
	    std::string curr_directory2 = get_current_dir_name();

	    search(curr_directory2, extension2);
        ...
        thread_data_array[f].VTKinput = c;
